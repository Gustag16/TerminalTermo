...
// Número máximo de tentativas antes do usuário perder o jogo
...
// Verifica se o usuário acertou as 5 posições
...
// Apaga um certo número de linhas no terminal dependendo do parâmetro
...
// Função auxiliar que remove acentos em UTF-8
...
void ranqueie_jogador(int tentativas) {
    printf("Deseja entrar no ranking?\n");
    printf("Digite 1 para SIM\nDigite 2 para NÃO\n");
    printf("> ");

    char entrada[160];
    fgets(entrada, sizeof entrada, stdin);
    char selecao_ranking;
    sscanf(entrada, "%c", &selecao_ranking);

    do {
        if (selecao_ranking == 2) {
            apaga_linha(3);
            return;
        }
        else if (selecao ranking == 1) {
            apaga_linha(3);
            ...
            FILE *arquivo_ranking = fopen("arquivo_ranking", "a");
            fclose(arquivo_ranking);
            return;
        }
        else {
            printf(RED "Erro: escolha uma opção válida\n" RESET);
            apaga_linha(4);
        }
    } while (selecao_ranking != 1 || selecao_ranking != 2);
